...
// Generic API:
export function generateName(schemaName, gender) {
  const schema = NAME_SCHEMAS[schemaName.trim().toUpperCase()];
  if (!schema) throw new Error(`Unknown schema name "${schemaName}". Use one of: ${Object.keys(NAME_SCHEMAS).join(", ")}`);
